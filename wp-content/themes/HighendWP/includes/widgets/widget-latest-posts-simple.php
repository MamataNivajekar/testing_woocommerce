...
class hb_latest_posts_simple_widget extends WP_Widget {
	...
	function __construct() {
		...
		$control_ops = array ();
		/* Create the widget. */
		parent::__construct( 'hb_latest_posts_simple_widget', __('[HB-Themes] Simple Post List','hbthemes'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $blog_posts->have_posts() ) : ?>
			...
			while ( $blog_posts->have_posts() ) : $blog_posts->the_post();
				$excerpt = wp_trim_words( wp_strip_all_tags( apply_filters( 'the_content', get_the_content() ) ), 10, '...<br/><a class="hb-focus-color" href="'. get_permalink() .'">' . __("Read More", "hbthemes") . '</a>' );
